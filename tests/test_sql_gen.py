...
class TestGenerateSql:
    ...
    def test_generates_sql_for_simple_question(self):
        ...
        assert result.get("sql_query") == expected_sql
        assert result.get("error") is None
    ...
    def test_cleans_markdown_from_llm_output(self):
        ...
        assert result.get("sql_query") == "select * from students"
...
class TestExecuteSql:
    def test_valid_query_returns_rows(self):
        ...
        assert result.get("error") is None
        assert isinstance(result.get("sql_results"), list)
        assert len(result.get("sql_results")) == 2
        assert "name" in result.get("sql_results")[0]
    ...
    def test_aggregation_query(self):
        result = execute_sql(_make_state(sql_query="""
            ...
            """))
        assert result.get("error") is None
        assert len(result.get("sql_results")) == 1
        assert "avg_grade" in result.get("sql_results")[0]
